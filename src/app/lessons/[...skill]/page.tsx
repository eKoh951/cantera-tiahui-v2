...
import { fetchLessonData } from "@/app/lib/fetchers";
...
export default async function Lesson({
  ...
  if (!skillLesson) {
    const lessons = await fetchLessonData(`${CMS_LESSONS_URL}/${skillName}`);

    return Array.isArray(lessons) ? <LessonList lessons={lessons} /> : <></>;
  }

  const lesson = await fetchLessonData(
...
export async function generateStaticParams() {
  const lessons = await fetchLessonData(CMS_LESSONS_URL);
